...
def read_signal_route_obstacles(def_file, power_nets):
    shapes = {}
    current_net = None
    in_nets = False
    rect_re = re.compile(
    ...
    with open(def_file, "r") as f:
        for line in f:
            ...
            if current_net in power_nets:
                continue
            match = rect_re.search(stripped)
            if match:
                layer = match.group(1)
                rect = [int(match.group(i)) for i in range(2, 6)]
                shapes.setdefault(layer, []).append(rect)
    return shapes


def nearest_rail_virtual_pins(points_by_layer, endpoints, modules, scale, width):
    virtual_pins = []
    for endpoint in endpoints:
        module = modules[endpoint["module"]]
        pin_center = module["pins"][endpoint["pin_index"]]
        px, py = pin_center["x"], pin_center["y"]
        virtual_pin = {}
        for layer_num, points in points_by_layer.items():
            if not points:
                continue
            layer_name = inverse_layers[layer_num]
            access_width = layerSpecs.get(layer_name, {}).get("Width", width)
            half_width = max(1, int(round(access_width * scale / 2)))
            x, y = min(
                points,
                key=lambda point: (
                    point[0] * scale - px
            ...
            virtual_pin[layer_name] = [[
                cx - half_width, cy - half_width,
                cx + half_width, cy + half_width
            ]]
        virtual_pins.append(virtual_pin)
    return virtual_pins
...
def build_augmented_pdn_inputs(cfg, decap_refs, placement_output, lef_output):
    ...
    placement_input = os.path.join(
        project_dir, "stage_3", "placement", f"{top_name}_placement.json"
    )
    primitive_lef = os.path.join(
        project_dir, "stage_3", "routing", "lef",
        f"{top_name}_primitives.lef"
    )
    decap_name = "auto_gen_decap_lna_0_1"
    decap_gds = os.path.join("FIXED_PRIMITIVES", f"{decap_name}.gds")

    with open(placement_input, "r") as f:
        placement = json.load(f)

    decap_lib = gdspy.GdsLibrary(infile=decap_gds)
    decap_cell = decap_lib.cells[decap_name]
    bbox = decap_cell.get_bounding_box()
    pin_positions = {
        label.text.upper(): label.position
        for label in decap_cell.get_labels(depth=None)
    }
    pin_names = ["PLUS", "MINUS", "BULK1", "BULK2"]
    missing = [name for name in pin_names if name not in pin_positions]
    if missing:
    ...
    for index, ref in enumerate(decap_refs):
        ...
        for pin_name in pin_names:
            ...
            pins.append({
                "x": int(round((ox + px) * scale)),
                "y": int(round((oy + py) * scale))
            })
        placement["modules"][instance_name] = {
            "x": int(round(ox * scale)),
            "y": int(round(oy * scale)),
            "w": int(round((bbox[1][0] - bbox[0][0]) * scale)),
            "h": int(round((bbox[1][1] - bbox[0][1]) * scale)),
            "orientation": 0,
            "gds_file": decap_gds,
            "dummy_gds_file": None,
            "pins": pins
    ...
    with open(placement_output, "w") as f:
        json.dump(placement, f, indent=2)

    lef_dir = os.path.dirname(lef_output)
    converter = GDS2_LEF(cfg["pdk"], decap_gds, decap_name)
    converter.writeLEF(lef_dir + "/", scale, piniso=True)
    decap_lef = os.path.join(lef_dir, f"{decap_name}.lef")
    with open(lef_output, "w") as output:
        with open(primitive_lef, "r") as source:
            output.write(source.read())
        output.write("\n")
        with open(decap_lef, "r") as source:
            output.write(source.read())
    print(f"Saved augmented PDN placement: {placement_output}")
    print(f"Saved augmented PDN LEF: {lef_output}")
    return placement
...
def write_pdn_router_constraints(cfg, output_file, vdd_points, gnd_points,
    ...
    print(f"Saved PDN router constraints: {output_file}")
...
def main():
    ...
    parser.add_argument("--router-constraints", help="Generated PDN router constraints JSON")
    parser.add_argument("--signal-def", help="Existing signal-route DEF used as PDN obstacles")
    parser.add_argument("--placement-output", help="Augmented placement containing generated decaps")
    parser.add_argument("--lef-output", help="Augmented primitive LEF containing the decap master")
    parser.add_argument(
        "--io-direction",
        nargs="+",
        default=["N", "W", "E", "S"],
    ...
    decap_refs = place_decaps_from_grid_with_gds(
        ...
        num_decaps=cfg["pdn_routing"].get("decap_count", 3)
    )
    ...
    if args.router_constraints:
        if not args.signal_def or not args.placement_output or not args.lef_output:
            parser.error(
                "--signal-def, --placement-output, and --lef-output are required "
                "with --router-constraints"
            )
        placement = build_augmented_pdn_inputs(
            cfg, decap_refs, args.placement_output, args.lef_output
        )
        write_pdn_router_constraints(
            cfg, args.router_constraints, vdd_via_dict, gnd_via_dict,
            args.signal_def, placement
        )
