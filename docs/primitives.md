...
In the paper, this part of the flow is the **Primitive Block Layout Generator** stage.
...
The paper emphasizes that passive layout is not an afterthought. Inductors, transmission lines, CPWs, capacitors, and device blocks are translated into DRC-clean layout primitives early enough that placement and routing can make physically meaningful decisions.

That is also how this public repo is structured:

- stage 1 sizing decides target values
- primitive optimizers map targets to geometry
- primitive generators produce layout-ready GDS blocks
- placement and routing consume those blocks directly
...
This aligns especially closely with the paper's emphasis on multiple EM-equivalent inductor variants with different footprints and pin locations, which helps the placer choose the most layout-efficient realization.
