...
{
  PortCVec realports, virtualports;
  for (auto& p : _pins) {
    realports.insert(realports.end(), p->ports().begin(), p->ports().end());
  }
  for (auto& p : _vpins) {
    virtualports.insert(virtualports.end(), p->ports().begin(), p->ports().end());
  }
  if (!realports.empty() && realports.size() == virtualports.size()) {
    PortPairs localpairs;
    std::vector<bool> used(virtualports.size(), false);
    for (auto* realport : realports) {
      double mindist{1e30};
      int nearest{-1};
      for (unsigned i = 0; i < virtualports.size(); ++i) {
        if (used[i]) continue;
        const double dist = Geom::Dist(realport->bbox(), virtualports[i]->bbox());
        if (dist < mindist) {
          mindist = dist;
      ...
      if (nearest >= 0) {
        used[nearest] = true;
        localpairs.emplace_back(realport, virtualports[nearest]);
        COUT << "local power ports to route : " << realport->name() << ' '
             << virtualports[nearest]->name() << ' ' << mindist << '\n';
      }
    }
    return localpairs;
  }
