...
def create_tline(
    ...
    tline = gdspy.Cell(f"{gds_name}_dummy")
    ...
    # --- Add flexpath shield around TLINE ---
    shield_offset = 15 # spacing from TLINE edge
    shield_width  = 10    # shield line thickness

    # Coordinates
    x0, x1 = 0 + shield_width/2 + width, length-shield_width/2  - width
    y0 = -shield_offset
    y1 = width + shield_offset

    # Define closed rectangular path (clockwise)
    shield_points = [
        (x0-shield_width/2, y0),     # bottom-left
        (x1, y0),     # bottom-right
        (x1, y1),     # top-right
        (x0, y1),     # top-left
        (x0, y0)      # close path
    ...
    # Create single continuous flexpath
    for m in ["M1"]:
        shield_path = gdspy.FlexPath(
            shield_points,
            shield_width,
            corners='natural',       # ensures smooth 90° corners
            layer=layer_num[m],
            datatype=layer_datatypes[m]['Draw']
        )

        tline.add(shield_path)

    bbox = tline.get_bounding_box()
    if bbox is not None:
        llx, lly = bbox[0]
        dx, dy = -llx, -lly
    else:
    ...
    tline_to_export = gdspy.Cell(f'{gds_name}')
    tline_to_export.add(gdspy.CellReference(tline, (dx, dy)))


    out_path = os.path.join(gds_dir, f"{gds_name}.gds")
    gdspy.write_gds(out_path, cells=[tline, tline_to_export])
    # return mim_cap_cell

# --- Main execution block ---
def parse_args():
    parser = argparse.ArgumentParser(
        description="TRANSMISSION LINE GDS GENERATOR"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=100.0,
        help="Transmission line length in um (default: 100.0)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Transmission line width in um (default: 2.0)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="t_line",
        help="Output GDS base name without extension (default: t_line)"
    )
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("Transmission line GDS generator")
    print("=" * 80)
    ...
    print(f"  Custom  : python {script_name} --pdk /path/to/layers.json --length 200 --width 4 --output_dir PRIMITIVES --output_gds_name my_tline")
    print("=" * 80)
    ...
    print("\nGenerating transmission line layout...")
    create_tline(
        length=args.length,
        width=args.width,
